...
reviews_filename = './Data/sentiment_processed_data/reviews_cleaned.csv'
data = pd.read_csv(filename)
reviews = pd.read_csv(reviews_filename, names = ['listing_id', 'comments'])

# 将comments列转换为数字类型，无法转换的变为NaN
reviews['comments'] = pd.to_numeric(reviews['comments'], errors='coerce')
reviews['listing_id'] = pd.to_numeric(reviews['listing_id'], errors='coerce')

# 用0填充NaN
reviews['comments'] = reviews['comments'].fillna(0)
...
# 合并 data 和 reviews
data = pd.merge(data, reviews, left_on='id', right_on='listing_id', how='left')
