...
def get_ensemble_models():
    grad = GradientBoostingRegressor(n_estimators=17, random_state=42, loss='squared_error', learning_rate=0.12, max_depth=10)
...
def kmeans(X_train, y_train, X_val, y_val):
    ...
    for i in range(n_clusters):
        ...
        std_train = y_train[train_mask].std(axis=0)
        ...
        pred_mask = c_pred==i
        std_pred = y_val[pred_mask].std(axis=0)
        ...
        if (y_val_stats is None):
            ...
            y_val_stats = pd.concat([y_val_stats, y_val[pred_mask]], ignore_index=True)
            y_train_stats = pd.concat([y_train_stats, y_train[train_mask]], ignore_index=True)
            predicted_values = np.concatenate([predicted_values, y_pred])
            labels_stats = np.concatenate([labels_stats, labels_pred])
...
if __name__ == "__main__":
    ...
    coeffs = np.load('./Data/selected_coefs_lasso.npy', allow_pickle=True)
    col_set = set()
    cherry_picked_list = [
